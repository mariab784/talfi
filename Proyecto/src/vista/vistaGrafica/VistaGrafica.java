		...
		gordaLatex.addActionListener(new ActionListener(){
            ...
            public void actionPerformed(ActionEvent e) {
                class Pares {
                    ...
                    public Pares(String string, String string2) {
                    ...
                    public String getParLatex() {
                        return estadoL;
                    }
                }
                AutomataCanvas c = panelCentral.getCanvas();
                int numEst = c.getListaEstados().size();
                int numAristas = c.getListaAristas().size();
                int canvasX = c.getX();//o 30
                int canvasY = c.getY();// o 20
                int coorX;
                int coorY;
                int canvasYin = canvasY - (2*canvasY);
                String nombreEstado;
                boolean fin = false;
                int k = 0;
                String tipoAuto = null;
                String latex = null;
                Pares[] vector = new Pares[c.getListaEstados().size()];
                boolean enc = false;
                boolean enc2 = false;
                String par1 = null;
                String par2 = null;
                latex = "% Generated by Mario\n" +
                    "\\documentclass{article}\n" +
                    "\\usepackage[usenames]{color}% delete this line if you don't use colors\n\n" +
                    "\\usepackage{gastex}\n" +
                    "\\begin{document}\n\n" +
                    "\\begin{picture}("+canvasX+","+canvasY+")(0,"+canvasYin+")\n" +
                    "\\put(0,"+canvasYin+"){\\framebox("+canvasX+","+canvasY+"){}}\n\n";
                for(int i = 0; i < numEst; i++) {
                    nombreEstado = c.getListaEstados().get(i).getEtiqueta();
                    vector[i] = new Pares(nombreEstado,"n"+i);
                    coorX = c.getListaEstados().get(i).getX();
                    coorY = c.getListaEstados().get(i).getY();
                    fin = c.getListaFinales().contains(nombreEstado);
                    if(nombreEstado.equals(c.getEstadoInicial())) {
                        latex = latex + "\\node[Nfill=y,fillcolor=Goldenrod,NLangle=0.0,Nmarks=i](n"+i+")("+coorX+","+coorY+"){"+nombreEstado+"}\n";
                    }
                    else if(fin){
                        latex = latex + "\\node[Nfill=y,fillcolor=Goldenrod,NLangle=0.0](n"+i+")("+coorX+","+coorY+"){"+nombreEstado+"}\n"+
                                "\\rmark(n"+i+")\n";
                    }
                    else
                        latex = latex + "\\node[Nfill=y,fillcolor=Goldenrod,NLangle=0.0](n"+i+")("+coorX+","+coorY+"){"+nombreEstado+"}\n";
                }
                tipoAuto  = panelCentral.getPanel().getTipoAutomata();
                if((tipoAuto.equals("AutomataFD"))||(tipoAuto.equals("AutomataFND"))||(tipoAuto.equals("AutomataFNDLambda"))){
                    for(int j = 0; j < numAristas; j++) {
                        ...
                        if(c.getListaAristas().get(j).getOrigen().equals(c.getListaAristas().get(j).getDestino())){
                            k = 0;
                            while((!enc) && (k < numEst)) {
                                if(vector[k].getParEstado().equals(c.getListaAristas().get(j).getOrigen())){
                                    enc = true;
                                    latex = latex + "\\drawloop("+vector[k].getParLatex()+"){";
                                    latex = latex +c.getListaAristas().get(j).getEtiqueta()+"}\n";
                                }
                                else
                                    k++;
                            }
                            ...
                            enc = false;
                            enc2 = false;
                            while(((!enc) || (!enc2)) && (k < numEst)) {
                                if(vector[k].getParEstado().equals(c.getListaAristas().get(j).getOrigen())&&(!enc)) {
                                    par1 = vector[k].getParLatex();
                                    enc = true;
                                }
                                if(vector[k].getParEstado().equals(c.getListaAristas().get(j).getDestino())&&(!enc2)) {
                                    par2 = vector[k].getParLatex();
                                    enc2 = true;
                                }
                                else {
                                    k++;
                            ...
                            latex = latex + "\\drawedge[curvedepth=8.0]("+par1+","+par2+"){"+c.getListaAristas().get(j).getEtiqueta()+"}\n";
                        }
                    }
                }
                else if(tipoAuto.equals("AutomataPila")){
                    AutomataFD apila = new AutomataPila(c.getEstadoInicial(),c.getListaFinales(),c.getAlfabeto(),
                            c.getAlfabetoPila(),c.getNombreEstados(),c.getListaAristasPila());
                    ...
                    for(int j = 0; j < numAristas; j++) {
                        ...
                        if(c.getListaAristasPila().get(j).getOrigen().equals(c.getListaAristasPila().get(j).getDestino())){
                            k = 0;
                            while((!enc) && (k < numEst)) {
                                if(vector[k].getParEstado().equals(c.getListaAristasPila().get(j).getOrigen())){
                                    enc = true;
                                    latex = latex + "\\drawloop("+vector[k].getParLatex()+"){";
                                    latex = latex +c.getListaAristasPila().get(j).getEntradaSimbolos()+","+
                                            c.getListaAristasPila().get(j).getCimaPila()+","+
                                            c.getListaAristasPila().get(j).getSalidaPila()+"}\n";
                                }
                                else
                                    k++;
                            }
                            ...
                            enc2 = false;
                            while(((!enc) || (!enc2)) && (k < numEst)) {
                                if(vector[k].getParEstado().equals(c.getListaAristasPila().get(j).getOrigen())&&(!enc)) {
                                    par1 = vector[k].getParLatex();
                                    enc = true;
                                }
                                if(vector[k].getParEstado().equals(c.getListaAristasPila().get(j).getDestino())&&(!enc2)) {
                                    par2 = vector[k].getParLatex();
                                    enc2 = true;
                                }
                                else {
                            ...
                            latex = latex + "\\drawedge[curvedepth=8.0]("+par1+","+par2+"){"+c.getListaAristasPila().get(j).getEntradaSimbolos()+","+
                                    c.getListaAristasPila().get(j).getCimaPila()+","+
                                    c.getListaAristasPila().get(j).getSalidaPila()+"}\n";
                        }
                    }
                }
                else if(tipoAuto.equals("MaquinaTuring")){
                    MaquinaTuring mtur = new MaquinaTuring(c.getEstadoInicial(),c.getListaFinales(),c.getAlfabeto(),
                            c.getAlfabetoCinta(),c.getNombreEstados(),c.getListaAristasTuring());
                    ...
                    for(int j = 0; j < numAristas; j++) {
                        ...
                        if(c.getListaAristasTuring().get(j).getOrigen().equals(c.getListaAristasTuring().get(j).getDestino())){
                            k = 0;
                            while((!enc) && (k < numEst)) {
                                if(vector[k].getParEstado().equals(c.getListaAristasTuring().get(j).getOrigen())){
                                    enc = true;
                                    latex = latex + "\\drawloop("+vector[k].getParLatex()+"){";
                                    latex = latex +c.getListaAristasTuring().get(j).getEntradaCinta()+","+
                                            c.getListaAristasTuring().get(j).getSimboloCinta()+","+
                                            c.getListaAristasTuring().get(j).getDireccion()+"}\n";
                                }
                                else
                                    k++;
                            }
                            ...
                            enc2 = false;
                            while(((!enc) || (!enc2)) && (k < numEst)) {
                                if(vector[k].getParEstado().equals(c.getListaAristasTuring().get(j).getOrigen())&&(!enc)) {
                                    par1 = vector[k].getParLatex();
                                    enc = true;
                                }
                                if(vector[k].getParEstado().equals(c.getListaAristasTuring().get(j).getDestino())&&(!enc2)) {
                                    par2 = vector[k].getParLatex();
                                    enc2 = true;
                                }
                                else {
                            ...
                            latex = latex + "\\drawedge[curvedepth=8.0]("+par1+","+par2+"){"+c.getListaAristasTuring().get(j).getEntradaCinta()+","+
                                    c.getListaAristasTuring().get(j).getSimboloCinta()+","+
                                    c.getListaAristasTuring().get(j).getDireccion()+"}\n";
                        }
                    }
                    ...
                    System.out.println("Aut�mata no reconocido.");
                latex = latex + "\n\\end{picture}\n"+"\\end{document}\n";
