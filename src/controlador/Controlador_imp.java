...
public class Controlador_imp implements Controlador{
	...
	/**
	 * Constructor del Controlador vacío
	 */
	...
	/**
	 * Método singleton que obtiene la instancia del Controlador si 
	 * está creada y sino crea una nueva
	...
	private void minimizacion(Automata a,boolean pasos) throws AutomatasException {
		...
		if (aux2 instanceof AutomataFD && !(aux2 instanceof AutomataFND)) {
			...
			MinimizacionAFD algMinimizacion=new MinimizacionAFD(aux2);
			...
			xmlSalida=rutaxml;
			salida.toString();
			//se recargaría y se llmaría a la clase de pintado en la interfaz grafica con el objeto salida
			//eso fuera de ese metodo, sería despues del ejecuta query con la salida obtenida
		}
			else {
				mensajero=Mensajero.getInstancia();
