 ...
 * si el automata que se pasa no es determinista se deberá traducir a un afd
 * 
...
public class MinimizacionAFD implements Algoritmo{
	...
	private HashMap<String,HashMap<String,Boolean>> tabla;
	 ...
	 */
	public MinimizacionAFD(Automata ent) {
		mensajero=new Mensajero();
		this.automataEntrada=ent;
		tabla=new HashMap<String,HashMap<String,Boolean>>();
		numPasos=1;
	...
	private void inicializarTabla() {
		...
		while (iter.hasNext()){
			...
			HashMap<String,Boolean> fila=new HashMap<String,Boolean>();
	...
	*/
	...
	private void paso() {
		...
		while(it.hasNext()) {
			String v1=it.next();
			HashMap<String,Boolean> fila=tabla.get(v1);
				...
				if (tabla.get(v1).get(v2)) {
					...
					while(itArista.hasNext()) {
						...
						if ((v1_letra!=null)&&(v2_letra!=null)) {
							...
							Boolean estaMarcado=tabla.get(v1_letra).get(v2_letra);
							if (estaMarcado==null) estaMarcado=tabla.get(v2_letra).get(v1_letra);
							if (estaMarcado!=null) {
								//HAY QUE DEJAR CONSTANCIA DEL RESULTADO EN LA TABLA
								if (fila.get(v2)!=estaMarcado&&estaMarcado==false)hayCambios=true;
								System.out.println("Marco:["+v1+","+v2+"]");
								System.out.println("estaMarcado:"+estaMarcado);
								HashMap<String,HashMap<String,Boolean>> clon=(HashMap<String,HashMap<String,Boolean>>)tabla.clone();
								//fila.put(v2,estaMarcado);
								if (estaMarcado==false)fila.put(v2,estaMarcado);
								tabla.put(v1,fila);
	...
	private void generaAutomata() {
		...
		while(it.hasNext()) {
			String e1=it.next();
			HashMap<String,Boolean> fila=tabla.get(e1);
...
private Automata renombrarSufijos(Automata a) {
	...
	while(itEst.hasNext()) {
		...
		if (!estado.contains("[")) {
			...
			estadoN+="]";
			thash.put(estado,estadoN);
			if (a.getEstadoInicial().equals(estado)) automata.setEstadoInicial(estadoN);
			if (a.getEstadosFinales().contains(estado)) nEstadosFin.add(estadoN);
			nEstados.add(estadoN);
		}
	}
...
/**
 * Método accesor de la tabla de minimización del algoritmo
 * @return la tabla de minimización de tipo HashMap<String,HashMap<String,Boolean>>
 */
public HashMap<String,HashMap<String,Boolean>> getTablaMinimizacion() {
	return tabla;
}
...
private String traducirXML(Automata automata) {
	...
	public static void main(String[] args) {
		...
		MinimizacionAFD min=new MinimizacionAFD(a);
		min.ejecutar(true);
	}
