...
public class Automatas_equivalentes implements Algoritmo{
	...
	public Automata ejecutar(boolean muestraPasos) {
		...
		//se genera un "automata" no conexo con los dos autñmatas.
		...
		try {
			algMinimizacion = new MinimizacionAFD(aux);
		...
		HashMap<String,HashMap<String,Registro>> tablaMinimizacion=algMinimizacion.getTablaMinimizacion();
	...
	private Automata generaAutomataConjunto() {
		...
		//que en el fondo es la uniñn de dos tablas de transiciones delta.
	...
	/**
	 * Mñtodo que inicializa el primero de los dos automatas de los que
	 * se quiere hacer la equivalencia
	 * @param a automata nuevo que serñ el primero para ejecutar el algoritmo
	 */
	...
	/**
	 * Mñtodo que inicializa el segundo de los dos automatas de los que
	 * se quiere hacer la equivalencia
	 * @param a automata nuevo que serñ el segundo para ejecutar el algoritmo
	 */
	public void registraAutomata2(Automata a) {
		automata2=a;
	}
	
	private boolean existenLasParejas(HashMap<String,HashMap<String,Registro>> tabla) {
		...
		while(it.hasNext()){
			...
			HashMap<String,Registro> fc=tabla.get(estado);
			Iterator<String> itCl=fc.keySet().iterator();
			while(itCl.hasNext()) {
				String est2=itCl.next();
				if (fc.get(est2).getMarcado()) {
					String s1=estado;
					String s2=est2;
	...
	/**
	 * Mñtodo accesor del resultado del algoritmo
	...
	private String traducirTablaMinimizacion(HashMap<String,HashMap<String,Registro>> tabla) {
		...
		while(itc.hasNext()) {
			...
			while(itf.hasNext()) {
				...
				salida+="<fila><state>"+fila+"</state>"+"<compatible>"+hs.get(fila).getMarcado()+"</compatible></fila>";
				salida+="<fila><state>"+fila+"</state>"+"<pasos>"+hs.get(fila).getPaso()+"</pasos></fila>";
				salida+="<fila><state>"+fila+"</state>"+"<estados>"+hs.get(fila).getEstados()+"</estados></fila>";
			}
			salida+="</column>";
		}
		return salida+"</table>";
	...
	private HashMap<String,HashMap<String,Registro>> quitarSubIndices(HashMap<String,HashMap<String,Registro>> tm) {
		HashMap<String,HashMap<String,Registro>> nuevaTabla=new HashMap<String,HashMap<String,Registro>>();
		...
		while(itKey.hasNext()) {
			...
			HashMap<String,Registro> hs=tm.get(estado);
			HashMap<String,Registro> nhs=new HashMap<String,Registro>();
